...
NAMESPACE="${NAMESPACE:-ahbn-k6-exp10}"; RELEASE="${RELEASE:-ahbn}"; STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
...
[ -n "${IMAGE}" ] || fail "IMAGE must name the manually built K6 Exp10 image"
...
if [ "${MODE}" = smoke ]; then DEFAULT_ROOT="${ROOT_DIR}/outputs/k6_exp10_smoke-${STAMP}"; seeds=(42); else DEFAULT_ROOT="${ROOT_DIR}/outputs/k6_exp10_formal-${STAMP}"; seeds=(42 43 44 45 46); fi
...
shasum -a 256 app/k5_final_actuator_policy.py app/k5_final_actuator_runtime.py >"${RESULT_ROOT}/k5_immutable_hashes.txt"
...
[ "$(awk 'NR==1{print $1}' "${RESULT_ROOT}/k5_immutable_hashes.txt")" = 8c7a0658cd226d0349e3ed3c64c943887196fdee57d9ef06874fd8525b683cff ] || fail "K5 policy changed"
[ "$(awk 'NR==2{print $1}' "${RESULT_ROOT}/k5_immutable_hashes.txt")" = 1d95271079064b6c159fcb9d7b553c03cc8b6cb42893f1b8e92bf8f4b6e95a25 ] || fail "K5 runtime changed"
"${PYTHON}" -m pytest -q tests/test_k6_exp10.py tests/test_k5_final_actuator_gke.py
...
for seed in "${seeds[@]}"; do
  ...
  for algorithm in "${algorithms[@]}"; do
    ...
    "${PYTHON}" app/k6_exp10_tools.py config --base experiments/k6_exp10.yaml --out "${config}" --algorithm "${algorithm}" --seed "${seed}"
    "${PYTHON}" app/gen_topology.py --config "${config}" --out "${topology}"; generated+=("${topology}")
  done
  "${PYTHON}" app/k6_exp10_tools.py contract "${generated[@]}"
done
...
for seed in "${seeds[@]}"; do
  for algorithm in "${algorithms[@]}"; do
    ...
    "${PYTHON}" app/k6_exp10_tools.py run --run-dir "${run_dir}"
...
CURRENT_STAGE="analysis"; "${PYTHON}" scripts/k6_exp10_analysis.py --root "${RESULT_ROOT}" --mode "${MODE}"
...
trap - EXIT; restore_topology; trap - ERR; echo "K6 Exp10 ${MODE} complete: ${RESULT_ROOT}"
