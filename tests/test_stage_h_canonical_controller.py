...
def expected(d: float, l: float, u: float, c: float):
    ...
    fanout = round(max(2.0, min(4.0, 2.0 + 2.0 * weight)))
...
class StageHCanonicalControllerTests(unittest.TestCase):
    ...
    def test_h10_fanout_quantization_boundaries(self):
        # round(2.5) == 2 and round(3.5) == 4 under Python's ties-to-even rule.
        for weight, fanout in ((.25 - 1e-9, 2), (.25, 2), (.25 + 1e-9, 3),
                               (.75 - 1e-9, 3), (.75, 4), (.75 + 1e-9, 4)):
            score = math.log(weight / (1 - weight))
            # Mapping-only state: scores below -1 are outside the canonical hat cube.
            state = AHBNState(d_hat=-score / .7 if score < 0 else 0,
                              l_hat=score / .7 if score >= 0 else 0)
            got = CanonicalAHBNController().update(state, 0, 0, 0, 0)
            self.assertAlmostEqual(got.score, score, delta=TOL)
            self.assertAlmostEqual(got.weight, weight, delta=TOL)
            self.assertEqual(got.fanout, fanout)
        canonical_corners = [self.assert_case(v) for v in product((0.0, 1.0), repeat=4)]
        self.assertNotIn(2, [r.fanout for r in canonical_corners])
