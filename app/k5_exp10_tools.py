...
def write_config(base,out,algorithm,seed):
    ...
    cfg=yaml.safe_load(base.read_text()); cfg["experiment"]=f"k5_exp10_{algorithm}_seed{seed}"
    cfg["strategy"]=STRATEGIES[algorithm]; cfg["topology"]["seed"]=seed
    source=choose_exp10_source(cfg); cfg["messageSource"]=source
    cfg["k5_exp10"]={"algorithm":algorithm,"seed":seed,"failure":"pod_delete","recovery_definition":"survivor_full_delivery","source_selection":"lowest_id_not_structured_ch_or_dcsoc_core"}
...
def validate_topologies(paths,expected_algorithms):
    ...
    for path in paths:
        topo=json.loads(path.read_text()); meta=topo["k5_exp10"]; algorithm=meta["algorithm"]; observed.append(algorithm)
...
def validate_run(run_dir):
    ...
    topo=json.loads((run_dir/"topology.json").read_text()); rows=load_jsonl(run_dir/"logs.jsonl"); algorithm=topo["k5_exp10"]["algorithm"]
    ...
    result={"run_id":topo["run_id"],"algorithm":algorithm,"seed":topo["k5_exp10"]["seed"],"target_peer_id":target,"target_role":selected[0]["role"],"target_cluster":selected[0].get("cluster_id"),"target_degree":selected[0]["degree"],
